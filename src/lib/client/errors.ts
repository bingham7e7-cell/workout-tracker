...
export function describeError(error: unknown): string {
  ...
  if (e?.code === "28000" || e?.code === "42501" || /jwt|not signed in|auth|permission denied/i.test(message)) {
...
export function isSignedOutError(error: unknown): boolean {
  ...
  return e?.code === "28000" || e?.code === "42501" || /jwt|not signed in|permission denied/i.test(e?.message ?? "");
}
